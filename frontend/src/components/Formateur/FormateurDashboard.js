...
const FormateurDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("formationStatusUpdated", (formation) => {
      message.info("Le statut de votre formation a été mis à jour !");
  ...
  const handleNotificationItemClick = (notification) => {
    if (notification.type === "formationStatusUpdated") {
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      ...
      <Container sx={{ flexGrow: 1, p: 3, backgroundColor: "#f4f6f8" }}>
        <Box sx={{ my: 4 }}>
          ...
          <Typography>
            Bienvenue sur votre tableau de bord, Formateur. Gérez vos formations
            ici.
          </Typography>
              ...
              renderItem={(notification) => (
                ...
                >
                  {notification.type === "formationStatusUpdated" && (
                    <div>
                      Statut de votre formation mis à jour :{" "}
